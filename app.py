...
def fetch_analysis(ticker):
    ...
    try:
        stock = yf.Ticker(sym)
        # Fetch 5 years to ensure we have enough data for a 200-MA and a 3-year slice
...
if ticker_input:
    with st.spinner(f"Analyzing {ticker_input}..."):
        ...
        if error:
            ...
            with c4:
                st.markdown(f'<div class="stat-box"><p class="metric-label">Debt/Equity</p><p class="metric-value">{debt/100:.2f if debt else "N/A"}</p></div>', unsafe_allow_html=True)

            # 2. 3-YEAR TREND ANALYSIS
            st.subheader("📈 Institutional Trend Analysis (3-Year Context)")
            
            # Calculate 200-MA on the full 5Y dataset for accuracy
            df['MA200'] = df['Close'].rolling(window=200).mean()
            
            # FIXED: Filtering for last 3 years using modern Pandas logic
