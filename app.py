...
if 'selected_index' in st.session_state:
    ...
    fig_idx.update_layout(template="plotly_dark", height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='#1e222d'))
...
if selected == "Analysis":
    ...
    if search:
        ...
        if not hist.empty:
            ...
            pledged, hold_insider = info.get('pledgedPercent', 0) or 0, info.get('heldPercentInsiders', 0) * 100
...
# --- AI CAPITAL PLANNER ---
    ...
    with col_input:
        ...
        s_ratio, l_ratio, exp_return = config[risk]["sip"], config[risk]["lump"], config[risk]["return"]
        sip_total, lump_total = total_cap * s_ratio, total_cap * l_ratio
    ...
    with col_output:
        ...
        n, r_m = horizon * 12, exp_return / 12
        fv_sip = monthly_sip * (((1 + r_m)**n - 1) / r_m) * (1 + r_m)
        fv_lump = lump_total * (1 + exp_return)**horizon
        total_fv = fv_sip + fv_lump
        st.divider()
        st.markdown(f'<div class="metric-card"><small>ESTIMATED TOTAL VALUE AFTER {horizon} YEAR(S)</small><br><span style="font-size:28px; color:#c8a45e">₹{total_fv:,.0f}</span><br><small style="color:#888">Approx. Net Gain: ₹{total_fv - total_cap:,.0f} ({((total_fv/total_cap)-1)*100:.1f}%)</small></div>', unsafe_allow_html=True)
